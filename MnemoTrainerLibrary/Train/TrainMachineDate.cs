...
{
    public class TrainMachineDate
    {
        #region Внутренние поля.

        QuestionGeneratorDate generator = new QuestionGeneratorDate();
        ...
        public int MinYear
        {
            get { return this.generator.MinYear; }
            set
            {
                if (value <= 0)
                {
                ...
                bool changed = generator.MinYear != value;

                generator.MinYear = value;
                if (changed)
                {
                    this.settingsIsChanged();
                }
        ...
        public int MaxYear
        {
            get { return this.generator.MaxYear; }
            set
            {
                if (value <= 0)
                {
                ...
                bool changed = this.generator.MaxYear != value;

                this.generator.MaxYear = value;
                if (changed)
                {
                    this.settingsIsChanged();
                }
        ...
        private void CheckAnswer(bool autoTimer, string userAnswer)
        {
            ...
            if (this.generator.TestType == TrainTypeDate.DayOfWeek || this.generator.TestType == TrainTypeDate.MonthDate)
            {
                logDate = this.generator.QuestionDate;
            }

            currentSerie.Attempts.Add(new ExerciseAttemptDate(logDate, this.testStartDate, time, autoTimer, this.generator.QuestionText, this.generator.AnswerText, isAnswerRight, userAnswerText));
        ...
        {
            ExerciseTypeDate exerciseTypeDate = new ExerciseTypeDate();
        ...
        public string GetNameDayOfWeekByDate(DateTime date)
        {
            this.Stop();

            return date.ToLongDateString() + "\r\n" + GetNameDayOfWeek(date.DayOfWeek);
        }
    }
}
