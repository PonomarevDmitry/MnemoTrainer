...
{
    public class TrainMachineAssociationPair
    {
        ...
        private QuestionGeneratorAssociationPair generator = new QuestionGeneratorAssociationPair();
        ...
        public event FirstWordsEventHandler TrainFirstWords;
        ...
        protected void OnTrainFirstWords(FirstWordsEventArgs args)
        {
            if (TrainFirstWords != null)
            {
                TrainFirstWords(this, args);
            }
        }
        ...
        {
            get { return this.generator.WordsInQuestion; }
            set
            {
                if (value <= 0)
                {
                ...
                bool changed = this.generator.WordsInQuestion != value;

                this.generator.WordsInQuestion = value;
                if (changed)
                {
                    this.settingsIsChanged();
                }
        ...
        public TrainMachineAssociationPair()
        {
            ...
            {
                {
                    ...
                    {
                        string nextWord = nextQuestion.ToStringColumn();

                        if (this.WithNumber)
                        {
                            this.questionText = string.Format("{0}. {1}", nextQuestion.Number.ToString(), nextWord);
                        }
                        else
                        {
                            this.questionText = nextWord;
                        }
                    }
                    else
                    {
        ...
        private void GetFirstLines()
        {
            ...
            {
                if (item.QuestionSerie.Count > 0)
                {
                    args.Add(item.QuestionSerie[0].ToString());
                }
            }

            status = MachineStatus.WaitingAnswer;
            OnTrainFirstWords(new FirstWordsEventArgs(args));
        }
        ...
        public bool CanGetFirstLines
        {
            get { return this.status == MachineStatus.Pause; }
        }
        ...
        private void SaveExeruciseSerie()
        {
            ...
            ExerciseAttemptAssociationsPair attempt = new ExerciseAttemptAssociationsPair(testStartDate, testEndDate, testCheckDate, time, timeCheck);
            ...
            attempt.WordInQuestion = this.WordsInQuestion;
            ...
            ExerciseSerie currentSerie = new ExerciseSerie(new ExerciseType(TrainType.AssociationsPair));
