...
{
    /// <summary>
    /// Попытка выполнения тренировки парные ассоциации.
    /// </summary>
    public class ExerciseAttemptAssociationsPair : ExerciseAttemptAssociations
    {
        public int WordInQuestion { get; set; }

        public bool WithNumber { get; set; }
        public bool Random { get; set; }

        public ExerciseAttemptAssociationsPair()
        {
            this.type = TrainType.AssociationsPair;
        }

        public ExerciseAttemptAssociationsPair(DateTime dateStart, DateTime dateEnd, DateTime dateCheck, double time, double timeCheck)
        {
            this.type = TrainType.AssociationsPair;
        ...
        private const string xmlFieldWordInQuestion = "WordInQuestion";
        ...
        public override void LoadAttributesFromXml(XmlNode xmlNode)
        {
            ...
            attr = xmlNode.Attributes[xmlFieldWordInQuestion];
            if (attr != null && !string.IsNullOrEmpty(attr.Value))
            {
                int temp;
                if (int.TryParse(attr.Value, out temp))
                {
                    this.WordInQuestion = temp;
                }
            }
        ...
        public override XmlNode CreateXmlNode(XmlDocument doc)
        {
            ...
            attr = doc.CreateAttribute(xmlFieldWordInQuestion);
            attr.Value = this.WordInQuestion.ToString();
        ...
        public override void CreateTestTextFile()
        {
            if (this.Questions.Count > 0)
            {
                ...
                sb.AppendLine(string.Format("Тест \"Парные ассоциации\" на {0} связок по {1} слов.", this.Questions.Count, this.WordInQuestion.ToString()));
                sb.AppendLine();
                ...
                string fileName = "Тест " + this.DateStart.Value.ToString("yyyy-MM-dd HH-mm-ss") + " Парные ассоциации " + this.Questions.Count.ToString() + " по " + this.WordInQuestion.ToString() + ".txt";
